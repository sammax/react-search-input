(() => {
  ...
  var Space = Symbol("space");
  var Text = Symbol("text");
  var Numeric = Symbol("number");
  var Enum = Symbol("enum");
  var OptionName = Symbol("option name");
  var InvalidOptionName = Symbol("invalid option name");
  var EscapeSequence = Symbol("escape sequence");
  var InvalidEscapeSequence = Symbol("invalid escape sequence");
  var MissingQuote = Symbol("missing quote");
  var MissingOptionValue = Symbol("missing option value");
  ...
  function parse(str, config) {
      ...
      if (currentValue.length === 0) {
        ...
        if (!invalidValue) {
          if (currentOption.multiple) {
            ...
            result.options[currentOption.name].values.push(value);
          } else {
    ...
    function completeToken(type) {
      let suggest;
      if (inOptionValue || type === MissingOptionValue) {
        suggest = currentOption;
    ...
    function parseError(position, text) {
      if (error === null) {
        error = {position, text};
      }
    }
  ...
  function AutoCompleteEntry(props) {
    ...
    }, /* @__PURE__ */ import_react.default.createElement("div", null, props.name), /* @__PURE__ */ import_react.default.createElement("div", null, props.description));
  }
  function AutoComplete(props) {
    const [leftPos, setLeftPos] = (0, import_react.useState)(0);
    (0, import_react.useEffect)(() => {
      setLeftPos(props.anchor.offsetLeft - props.anchor.parentElement.offsetLeft);
    });
    return /* @__PURE__ */ import_react.default.createElement("div", {
      style: {position: "absolute", left: leftPos},
      className: "autocomplete-container"
    }, props.entries.map((entry, index) => /* @__PURE__ */ import_react.default.createElement(AutoCompleteEntry, {
  ...
  function Search(props) {
    ...
    async function handleInput() {
      ...
      if (anchorPos === -1 || focusPos === -1) {
        anchorPos = focusPos = this.textContent.length;
      }
      ...
      if (result.tokens.length > 0) {
        for (const token of result.tokens) {
          const content = token.content.replace(/\s/g, String.fromCharCode(160));
          ...
          if (startPos >= currentOffset && startPos < currentOffset + length) {
            range.setStart(tokenElem.firstChild, startPos - currentOffset);
          }
          if (endPos >= currentOffset && endPos < currentOffset + length) {
            range.setEnd(tokenElem.firstChild, endPos - currentOffset);
            if (startPos === endPos) {
              autocompleteAnchor = tokenElem;
        ...
        if (startPos >= currentOffset) {
          const node = this.lastChild?.firstChild;
    ...
    (0, import_react.useEffect)(installInputEventListener);
    function suggestionUp() {
      if (autocomplete !== null) {
        let selection = (autocomplete.entries.length + autocomplete.selection - 1) % autocomplete.entries.length;
        setAutocomplete({
          ...autocomplete,
          selection
        });
    ...
    function suggestionSelect() {
      if (autocomplete !== null) {
        ...
        r.setStart(el.firstChild, val.length);
        r.setEnd(el.firstChild, val.length);
        sel.removeAllRanges();
        sel.addRange(r);
        handleInput.call(inputRef.current);
      }
    }
    ...
    return /* @__PURE__ */ import_react.default.createElement("div", {
      ...
      ref: inputRef,
      style: {display: "block"}
    }), autocomplete && /* @__PURE__ */ import_react.default.createElement(AutoComplete, {
      entries: autocomplete.entries,
      anchor: autocomplete.anchor,
      selected: autocomplete.selection,
      onHover: suggestionHover,
      onSelect: (idx) => {
        suggestionHover(idx);
        suggestionSelect();
      }
  ...
  var searchProps = {
    ...
    onError(err) {
      ...
      outputContainer.innerText = `${err.text} at character ${err.position}`;
    },
    options: [
      {
        name: "aNumber",
      ...
      },
      {
        name: "anEnum",
        type: "enum",
        options: ["foo", "bar", "hello", "world", "lorem", "ipsum"]
      },
      {
        name: "aBool",
        type: "boolean"
      },
      {
        ...
        suggestionProvider(search) {
          if (search.length > 0) {
            return fetch(`https://api.github.com/search/repositories?q=${search}&per_page=7`).then((r) => r.json()).then(({items}) => {
              if (items) {
  ...
  import_react_dom.default.render(/* @__PURE__ */ React2.createElement(React2.StrictMode, null, /* @__PURE__ */ React2.createElement(Search, {
    ...searchProps
